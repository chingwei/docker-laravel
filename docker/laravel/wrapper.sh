...
status=$?
if [ $status -ne 0 ]; then
  echo "cron Failed: $status"
  exit $status
  else echo "Starting Cron: OK"
fi
...
while /bin/true; do
  ...
  ps aux | grep '/usr/sbin/cron' | grep -q -v grep
  CRON_STATUS=$?
  echo "Checking CRON, Status Code: $CRON_STATUS"
  sleep 2
  ...
  if [ $PHP_FPM_STATUS -ne 0 ];
    then
     ...
     then
       ...
       exit -1
   elif [ $CRON_STATUS -ne 0 ];
     then
       echo "$(date +%F_%T) FATAL: CRON Raised a Status Code of $CRON_STATUS and exited"
       exit -1
   else
     sleep 2
